...
extern "C"
{
	...
	void CAPI_DECLSPEC Config(int nListenPort)
	{
		...
		CEasylog::GetInstance()->Init("network_core.log", Loglvel::LOGLEVEL_DEBUG);
	...
	void CAPI_DECLSPEC RegisterLogHandler(LogHandler handler)
	{
		CEasylog::GetInstance()->RegisterHandler(handler);
	};
}