...
public:
	static int UpdatePlaying(lua_State* L)
	{
		...
		CServer::GetInstance()->UpdatePlaying(uid);
		return 0;
	}
	static int	UpdateExited(lua_State* L)
	{
		int uid = (int)lua_tonumber(L, -1);
		CServer::GetInstance()->UpdateExited(uid);
		return 0;
	}
	static int UpdateUserInfo(lua_State* L)
	{
		...
		CServer::GetInstance()->UpdateUserInfo(uid,info);
		return 0;
	}
	static int SendData(lua_State* L)
	{
		...
		CServer::GetInstance()->SendData(uid, cmd, message);
		return 1;
	}
